...
def _convert_pdf_with_grobid(
    *,
    ...
    base_url = str(os.environ.get(URL_TEXT_GROBID_URL_ENV) or "").strip().rstrip("/")
    if not base_url:
        raise UrlTextExtractionError(
            code="grobid_not_configured",
            message=f"Set {URL_TEXT_GROBID_URL_ENV} to enable GROBID PDF extraction.",
        )
