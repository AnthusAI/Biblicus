...
# Configuration: PaddleOCR layout detection → Tesseract with layout metadata
config = {
...
layout_step_dir = snapshot.snapshot_dir / "steps" / "01-paddleocr-layout"
        ...
        if metadata_files:
            ...
            if metadata.get('regions'):
                ...
                for region in metadata['regions'][:3]:  # Show first 3
                    print(f"      - {region['type']}: order={region['order']}, bbox={region['bbox'][:2]}...")
    else:
...
# Check OCR step
ocr_step_dir = snapshot.snapshot_dir / "steps" / "02-ocr-tesseract"
if ocr_step_dir.exists():
    print("✓ OCR step exists")

    # Check text output
    text_dir = snapshot.snapshot_dir / "text"
    if text_dir.exists():
