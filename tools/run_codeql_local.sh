...
usage() {
  cat <<'EOF'
Run CodeQL code-quality analysis locally (mirrors GitHub "CodeQL" check)
...
fetch_codeql() {
  ...
  if [[ -x "$CODEQL_BIN" ]]; then
    CODEQL="$CODEQL_BIN"
    log "Using downloaded CodeQL: $CODEQL"
    return
  fi
  ...
  case "$(uname -s)" in
    ...
    *)
      err "Unsupported platform for CodeQL auto-download"
      exit 1
      ;;
  esac
  ...
  if [[ ! -x "$extracted_bin" ]]; then
    err "Downloaded bundle missing binary at $extracted_bin"
    exit 1
  fi
...
run_lang() {
  ...
  log "Creating database for ${lang}"
  create_opts=(
    "$db_dir"
    --language="$lang_arg"
