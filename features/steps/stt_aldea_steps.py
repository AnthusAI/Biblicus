...
from biblicus.extractors.aldea_stt import (
    AldeaSpeechToTextExtractor,
    AldeaSpeechToTextExtractorConfig,
)
...
def _call_aldea_stt_extractor(
    *,
    workdir: Path,
    aldea_api_key: str | None,
) -> Exception | None:
    ...
    parsed_config = AldeaSpeechToTextExtractorConfig()
    ...
    prior_aldea_key = os.environ.pop("ALDEA_API_KEY", None)
    prior_home = os.environ.get("HOME")
    prior_cwd = os.getcwd()
    try:
        if aldea_api_key is not None:
            os.environ["ALDEA_API_KEY"] = aldea_api_key
        ...
        if prior_aldea_key is not None:
            os.environ["ALDEA_API_KEY"] = prior_aldea_key
        else:
...
def step_call_aldea_stt_extractor_without_api_key(context) -> None:
    context.extraction_fatal_error = _call_aldea_stt_extractor(
...
def step_call_aldea_stt_extractor_with_api_key(context) -> None:
    context.extraction_fatal_error = _call_aldea_stt_extractor(
        workdir=Path(context.workdir),
        aldea_api_key="test-aldea-key",
    )
