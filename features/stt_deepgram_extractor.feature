Feature: Deepgram speech to text extraction
  ...
  Scenario: Deepgram speech to text stores structured metadata for downstream stages
    Given I initialized a corpus at "corpus"
    And a fake Deepgram library is available that returns transcript "Hello from Deepgram" for filename "clip.wav"
    And a Deepgram API key is configured for this scenario
    And a file "clip.wav" exists with bytes:
    ...
    And I build a "pipeline" extraction snapshot in corpus "corpus" with stages:
      ...
      | stt-deepgram | {}          |
    Then the extraction stage metadata for stage 1 includes key "deepgram"

  Scenario: Deepgram transform stage can render words
    Given I initialized a corpus at "corpus"
    And a fake Deepgram library is available that returns transcript "Hello from Deepgram" for filename "clip.wav"
    And a Deepgram API key is configured for this scenario
    And a file "clip.wav" exists with bytes:
