Feature: Markov analysis schema validation
  Markov analysis configurations are strict and raise explicit errors on invalid inputs.

  Scenario: Markov segmentation config requires llm configuration when method is llm
    When I attempt to validate a Markov configuration with segmentation method "llm" and no llm config
  ...
  Scenario: Markov segmentation config requires span markup configuration when method is span markup
    When I attempt to validate a Markov configuration with segmentation method "span_markup" and no span markup config
    Then a model validation error is raised
  ...
  Scenario: Markov span markup system prompt requires text placeholder
    When I attempt to validate a Markov configuration with span markup system prompt missing "{text}"
    Then a model validation error is raised
  ...
  Scenario: Markov span markup prompt template must not include text placeholder
    When I attempt to validate a Markov configuration with span markup prompt template containing "{text}"
    Then a model validation error is raised
  ...
  Scenario: Markov span markup requires label attribute when prepend label is enabled
    When I attempt to validate a Markov configuration with span markup prepend label enabled and no label attribute
    Then a model validation error is raised
  ...
  Scenario: Markov span markup end label value requires verifier
    When I attempt to validate a Markov configuration with span markup end label value but no verifier
    Then a model validation error is raised
  ...
  Scenario: Markov span markup end reject label value requires verifier
    When I attempt to validate a Markov configuration with span markup end reject label value but no verifier
    Then a model validation error is raised
    And the validation error mentions "segmentation.span_markup.end_label_verifier"

  Scenario: Markov span markup end label verifier system prompt requires text placeholder
    When I attempt to validate a Markov configuration with end label verifier system prompt missing "{text}"
    Then a model validation error is raised
  ...
  Scenario: Markov span markup end label verifier prompt template must not include text placeholder
    When I attempt to validate a Markov configuration with end label verifier prompt template containing "{text}"
    Then a model validation error is raised
  ...
  Scenario: Markov state naming requires client and prompts when enabled
    When I attempt to validate a Markov configuration with state naming enabled and missing requirements
    Then a model validation error is raised
  ...
  Scenario: Markov state naming requires system prompt when enabled
    When I attempt to validate a Markov configuration with state naming enabled and missing system prompt
    Then a model validation error is raised
  ...
  Scenario: Markov state naming requires prompt template when enabled
    When I attempt to validate a Markov configuration with state naming enabled and missing prompt template
    Then a model validation error is raised
  ...
  Scenario: Markov state naming system prompt requires context pack placeholder
    When I attempt to validate a Markov configuration with state naming system prompt missing "{context_pack}"
    Then a model validation error is raised
  ...
  Scenario: Markov state naming prompt template must not include context pack placeholder
    When I attempt to validate a Markov configuration with state naming prompt template containing "{context_pack}"
    Then a model validation error is raised
    And the validation error mentions "state_naming.prompt_template"

  Scenario: Markov topic modeling requires a configuration when enabled
    When I attempt to validate a Markov configuration with topic modeling enabled and no configuration
    Then a model validation error is raised
    And the validation error mentions "topic_modeling.configuration is required"

  Scenario: Markov topic modeling requires single LLM extraction method
    When I attempt to validate a Markov configuration with topic modeling enabled and non-single LLM extraction
    Then a model validation error is raised
    And the validation error mentions "topic_modeling.configuration.llm_extraction.method must be 'single'"
  ...
  Scenario: Markov llm observations require client and prompt template when enabled
    When I attempt to validate a Markov configuration with llm observations enabled and missing requirements
    Then a model validation error is raised
  ...
  Scenario: Markov llm observations require prompt template when enabled
    When I attempt to validate a Markov configuration with llm observations enabled and empty prompt template
    Then a model validation error is raised
  ...
  Scenario: Markov embeddings validate text source
    When I attempt to validate a Markov configuration with embeddings enabled and invalid text source
    Then a model validation error is raised
  ...
  Scenario: Markov observations validate text source
    When I attempt to validate a Markov configuration with observations text_source "unknown"
    Then a model validation error is raised
    And the validation error mentions "observations.text_source"

  Scenario: Markov configuration rejects unsupported schema versions
    When I attempt to validate a Markov configuration with schema version 2
    Then a model validation error is raised
  ...
  Scenario: Markov tfidf config rejects invalid ngram ranges
    When I attempt to validate a Markov configuration with tfidf ngram_range null
    Then a model validation error is raised
    And the validation error mentions "ngram_range"
    When I attempt to validate a Markov configuration with tfidf ngram_range [1]
    Then a model validation error is raised
    And the validation error mentions "tfidf.ngram_range must be"
    When I attempt to validate a Markov configuration with tfidf ngram_range [1, "a"]
    Then a model validation error is raised
    And the validation error mentions "tfidf.ngram_range must be"
    When I attempt to validate a Markov configuration with tfidf ngram_range [2, 1]
    Then a model validation error is raised
    And the validation error mentions "tfidf.ngram_range must be"
