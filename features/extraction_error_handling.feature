Feature: Extraction error handling
  An extraction snapshot should continue when an extractor fails for a single item.
  ...
  Scenario: Extraction continues and records an error when an item has invalid text encoding
    ...
    And the extraction snapshot error type for the first ingested item equals "UnicodeDecodeError"
  ...
  Scenario: Errored items do not create extracted text artifacts
    ...
    And the extraction snapshot includes an errored result for the last ingested item
