Feature: Retrieval evaluation
  ...
  Scenario: Evaluate a scan retriever snapshot against a tiny dataset
    ...
    And I evaluate the latest snapshot with dataset "dataset.json" and budget:
      | key                  | value |
      | max_total_items      | 3     |
  ...
  Scenario: Evaluate a snapshot with an unmatched expectation
    Given I initialized a corpus at "corpus"
    And a text file "one.md" exists with contents "alpha apple"
    When I ingest the file "one.md" into corpus "corpus"
    And I build a "scan" retrieval snapshot in corpus "corpus"
  ...
  Scenario: Evaluate a full-text search snapshot and report index size
    Given I initialized a corpus at "corpus"
