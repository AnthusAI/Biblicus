Feature: Embeddings generation (unit)
  Non-integration coverage for provider errors, empty inputs, and dependency checks.
  ...
  Scenario: Embeddings generator supports a single text convenience API
    Given a fake OpenAI library is available that returns embedding vector "5.0,6.0" for input text "alpha"
