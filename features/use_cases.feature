Feature: Use cases
  ...
  Scenario: Build a context pack from a folder of notes
    When I run the use case demo script "notes_to_context_pack_demo.py"
  ...
  Scenario: Ingest a folder of text files, extract, index, and retrieve evidence
    When I run the use case demo script "text_folder_search_demo.py"
    Then the demo output includes evidence
  ...
  Scenario: Mark sensitive spans for redaction (mock mode)
    When I run the use case demo script "text_redact_demo.py" with arguments:
      | arg    | value  |
      | --mock | true   |
