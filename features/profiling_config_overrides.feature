Feature: Profiling config overrides
  Profiling analysis accepts --config overrides even when no recipe file is provided.

  Scenario: Profiling analysis applies --config overrides without a recipe file
    ...
    And I build a "pipeline" extraction run in corpus "corpus" with steps:
      ...
      | pass-through-text | {}          |
    When I run a profiling analysis in corpus "corpus" using the latest extraction run with config overrides:
      | key                | value |
